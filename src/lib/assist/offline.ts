...
function answer(question: string, hits: Hit[]): string {
  ...
  if (hits.length === 0) {
    return [
      ...
      '',
      'Set `ANTHROPIC_API_KEY` and turn preview mode off to ask the model itself.',
    ].join('\n');
  }
  ...
  return [
    '**This is a sample answer. There is no model behind it.**',
    '',
    `Preview mode is on and no Anthropic key is set, so instead of an answer to “${asked}”, here is what the assistant would have been given to work from. A real answer would be written from these, in prose, with any figures coming from the calculators.`,
    '',
    ...hits.flatMap((hit) => [
      `### ${hit.doc.title}`,
      '',
