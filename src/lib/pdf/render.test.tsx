...
/**
 * Reconstruct the words actually printed on the page.
 *
 * react-pdf shows text as hex-encoded glyph strings (`<4649> Tj`) rather than
 * literal `(FI)` ones, even for the standard fonts — WinAnsiEncoding happens
 * to agree with ASCII in the printable range, so decoding each hex pair as a
 * byte recovers the original characters. `extractText` above only unescapes
 * parenthesised strings, which is why it cannot see this at all; this walks
 * every BT…ET text block and decodes both string forms in the order they're
 * shown, which is what lets a test assert on a word rather than an operator.
 */
function extractVisibleText(pdf: Buffer): string {
  const raw = pdf.toString('latin1');
  let content = '';
  for (const m of raw.matchAll(/stream\r?\n([\s\S]*?)\r?\nendstream/g)) {
    const body = Buffer.from(m[1], 'latin1');
    try {
      ...
      // Not a compressed stream (or not text) — nothing readable to add.
    }
  }
  let text = '';
  for (const block of content.matchAll(/BT([\s\S]*?)ET/g)) {
    for (const str of block[1].matchAll(/<([0-9A-Fa-f]+)>|\(((?:[^()\\]|\\.)*)\)/g)) {
      if (str[1]) {
        for (let i = 0; i < str[1].length; i += 2) {
          text += String.fromCharCode(parseInt(str[1].slice(i, i + 2), 16));
        }
...
describe('voucher PDF', () => {
  ...
  it('prints the client name, not the platform name, at the top of the document', async () => {
    const buf = await renderToBuffer(<VoucherDocument v={sample} />);
    const visible = extractVisibleText(buf);
    expect(visible).toContain('CIO Association');
    expect(visible).not.toContain('The Finance Intelligence');
  });

  it('no longer credits the desk in the page footer', async () => {
    const buf = await renderToBuffer(<VoucherDocument v={sample} />);
    const visible = extractVisibleText(buf);
    expect(visible).not.toContain('Generated by Voucher Desk');
  });
