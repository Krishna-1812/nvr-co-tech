 ...
 * distinct from `BRAND.name`, which names the Voucher Desk *platform* (used
 * in metadata like the PDF's `author`/`creator`, never on the printed page
 * itself).
 */
    ...
    >
      <Page size="A4" orientation="landscape" style={s.page}>
        <View style={s.frame}>
          ...
          <View style={s.header}>
            ...
            <View style={[s.titleBlock, { width: '40%' }]}>
              <Text style={s.firmName}>{VOUCHER_ORG_NAME}</Text>
        ...
        <View style={s.foot} fixed>
          <Text style={s.footText}>{v.voucher_no ?? 'Draft'}</Text>
          <Text
            style={s.footText}
