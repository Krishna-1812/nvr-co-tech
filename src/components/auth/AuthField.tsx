...
export function AuthInput({ icon: Icon, className, ...props }: InputProps) {
  /*
   * text-base below lg rather than 15px throughout: iOS Safari zooms the page
   * in whenever a field under 16px takes focus and never zooms back out, which
   * on a sign-in form leaves somebody panned sideways mid-password. `lg` is
   * where the signed-in side stops showing a touch dock, and a tablet has room
   * for the extra pixel in a card this width.
   */
  return (
    <div className="relative">
      <input
        {...props}
        className={cn(
          'peer h-12 w-full rounded-xl border border-[var(--m-line)] bg-white/[0.035] text-base lg:text-[15px] text-[var(--m-ink)] transition',
